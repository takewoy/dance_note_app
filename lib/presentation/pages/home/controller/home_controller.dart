...
final tabIndexProvider = StateProvider((_) => 0);

/// HomeTabBarView内のListViewでOverride
final indexedNoteProvider = Provider<DanceNote>((_) {
  throw UnimplementedError();
});

/// UI側では`danceNoteProvider`の状態を取得しないので、`AsyncLoading`かどうかを取得する
final isLoadingNoteProvider = Provider((ref) {
  return ref.watch(danceNoteProvider) is AsyncLoading;
});
...
final filteredDanceNoteProvider = Provider.family(
  (ref, DanceCategory category) {
...
final danceNoteProvider = StreamProvider<List<DanceNote>>((ref) async* {
